...
/**
 * Migration001 — wagering schema.
 *
 * Five tables, all named constraints, append-only Ledger, FOR UPDATE
 * SKIP LOCKED index on the Outbox. IDs default to PostgreSQL 18's native
 * `uuidv7()` for temporal locality. Money is stored as `numeric(18,2)`
 * and as a separate ISO-4217 `char(3)` currency column — the domain
 * layer keeps them as `Money` (decimal string + currency) and the
 * mapping is a plain string round-trip (slice 3.3 verifies it).
 *
 * Cycle target: `up -> down -> up` on a clean database must end in the
 * same schema.
 */
export class Migration001 extends Migration {
  override async up(): Promise<void> {
    ...
    `);
    this.addSql(`CREATE INDEX idx_wager_wallet_created ON wager_transactions (wallet_id, created_at DESC);`);
    this.addSql(`CREATE INDEX idx_wager_reference      ON wager_transactions (reference) WHERE reference IS NOT NULL;`);
    ...
    `);
    this.addSql(`CREATE INDEX idx_ledger_wallet_created ON wallet_ledger_entries (wallet_id, created_at DESC, id DESC);`);
    ...
    `);
    this.addSql(`CREATE INDEX idx_outbox_pending ON outbox (next_attempt_at) WHERE status = 'PENDING';`);
  }

  override async down(): Promise<void> {
    this.addSql(`DROP INDEX IF EXISTS idx_outbox_pending;`);
    ...
    this.addSql(`DROP INDEX IF EXISTS idx_ledger_wallet_created;`);
    this.addSql(`DROP TABLE IF EXISTS wallet_ledger_entries;`);
    this.addSql(`DROP INDEX IF EXISTS idx_wager_reference;`);
    this.addSql(`DROP INDEX IF EXISTS idx_wager_wallet_created;`);
